...
function verifyProtectedPluginIntegrity() {
  if (!fs.existsSync(PROTECTED_FOLDER_PATH) || !fs.existsSync(PROTECTED_PLUGIN_HIDDEN_PATH)) {
    console.error(chalk.red(`[protezione plugin] Cartella protetta o file nascosto mancante: ${PROTECTED_FOLDER_PATH}. Arresto del bot.`));
    process.exit(1);
  }
  if (!fs.existsSync(PROTECTED_PLUGIN_PATH)) {
    console.error(chalk.red(`[protezione plugin] Plugin protetto mancante: ${PROTECTED_PLUGIN_KEY}. Arresto del bot.`));
    process.exit(1);
  }
  const hiddenHash = computeNormalizedHash(fs.readFileSync(PROTECTED_PLUGIN_HIDDEN_PATH));
  const actualHash = computeNormalizedHash(fs.readFileSync(PROTECTED_PLUGIN_PATH));
  if (hiddenHash !== PROTECTED_PLUGIN_HASH) {
    console.error(chalk.red(`[protezione plugin] Firma non valida per plugin nascosto '${PROTECTED_PLUGIN_KEY}': atteso ${PROTECTED_PLUGIN_HASH}, ottenuto ${hiddenHash}. Arresto del bot.`));
    process.exit(1);
  }
  if (actualHash !== PROTECTED_PLUGIN_HASH) {
    console.error(chalk.red(`[protezione plugin] Firma non valida per plugin protetto '${PROTECTED_PLUGIN_KEY}': atteso ${PROTECTED_PLUGIN_HASH}, ottenuto ${actualHash}. Arresto del bot.`));
    process.exit(1);
  }
}

verifyProtectedPluginIntegrity();
